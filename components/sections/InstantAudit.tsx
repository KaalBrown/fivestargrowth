...
async function getAuditResult(url: string): Promise<AuditResult> {
  ...
  try {
    ...
    return { score: Math.round((performance + seo) / 2), performance, seo, source: "live" };
  } catch {
    return { score: fallbackScore, performance: fallbackScore, seo: Math.min(100, fallbackScore + 8), source: "estimated" };
  }
}
  ...
  const checks = useMemo(() => {
    ...
    return lowPerformance
      ...
      : [
          ...
          { title: "Google Maps & Local SEO Tags", detail: "Some local signals are present, with room to strengthen coverage.", icon: MapPin, status: "Partial", tone: "yellow" },
