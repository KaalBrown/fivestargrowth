...
export function InstantAudit() {
  ...
  const startAudit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const auditChecks = getAuditChecks(audit);
    const checkSummary = (title: string) => {
      const check = auditChecks.find((item) => item.title === title);
      return check ? `${check.status} — ${check.detail}` : "Not available";
    };
    const leadCaptureCheck = auditChecks.find((item) => item.title === "Lead & Review Capture Setup" || item.title === "Lead & Review Capture Paths");
    const email = String(formData.get("email") ?? "").trim();
    await deliverFormSubmission("New Five Star Growth instant website health score", {
      ...
      seo_score: `${audit.seo}%`,
      score_source: audit.source,
      speed_mobile_ux: checkSummary("Speed & Mobile UX"),
      google_maps_local_seo: checkSummary("Google Maps & Local SEO Tags"),
      lead_review_capture: leadCaptureCheck ? `${leadCaptureCheck.status} — ${leadCaptureCheck.detail}` : "Not available",
      marketing_consent: email ? "Yes — email provided for occasional score-improvement tips" : "No — no email provided",
    }).catch(() => undefined);
    setResult(audit);
    setIsScanning(false);
  };
        ...
        <div className="mt-12 grid overflow-hidden border border-black/20 bg-[#101010] text-white lg:grid-cols-[.9fr_1.1fr]">
          <div className="p-6 sm:p-8 lg:p-10">
            ...
            <form onSubmit={startAudit} className="mt-8 grid gap-3">
              ...
              <label><span className="sr-only">Email address</span><input name="email" type="email" autoComplete="email" placeholder="Email address (optional)" className="w-full border border-white/25 bg-white/[.04] px-4 py-3 text-sm outline-none placeholder:text-white/45 focus:border-[#ff5a1f]" /></label>
              <p className="-mt-1 text-xs leading-5 text-white/55">Optional: add your email for occasional practical tips to improve your local visibility and score. No spam—unsubscribe anytime.</p>
