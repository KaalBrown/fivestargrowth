...
const checkoutUrl = process.env.NEXT_PUBLIC_LOYALTY_STRIPE_PAYMENT_LINK;
...
export function LoyaltyProgramLanding() {
  ...
  return (
    <main className="overflow-hidden bg-[#080808] text-white selection:bg-orange-500 selection:text-black">
      ...
      <section id="checkout" className="scroll-mt-20 px-5 py-20 sm:px-8 sm:py-28"><div className="mx-auto max-w-5xl text-center"><p className="text-xs font-bold uppercase tracking-[.2em] text-orange-400">Start your scanner package</p><h2 className="mt-4 text-3xl font-black tracking-[-.05em] sm:text-5xl">Claim your Smart Scanner package today, 100% risk-free.</h2><div className="mt-6 flex flex-wrap justify-center gap-3"><span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/35 bg-emerald-400/10 px-4 py-2 text-sm font-bold text-emerald-200"><ShieldCheck size={16} /> 30-day money-back guarantee</span><span className="inline-flex items-center gap-2 rounded-full border border-orange-400/35 bg-orange-400/10 px-4 py-2 text-sm font-bold text-orange-200"><Truck size={16} /> Express 48h shipping</span></div></div><form onSubmit={handleCheckout} className="mx-auto mt-10 max-w-xl rounded-3xl border border-orange-500/45 bg-[#111] p-5 text-left shadow-[0_0_55px_rgba(255,107,0,.13)] sm:p-8"><div className="flex items-center justify-between border-b border-white/10 pb-5"><div><p className="font-black">Secure Stripe Checkout</p><p className="mt-1 text-sm text-zinc-400">Your card details are securely collected by Stripe.</p></div><CreditCard className="text-orange-400" /></div><div className="mt-6 grid gap-4"><label className="grid gap-2 text-sm font-bold text-zinc-200">Name on card<input required name="name" autoComplete="cc-name" placeholder="Your name" className="min-h-12 rounded-xl border border-white/15 bg-black px-4 text-white outline-none transition placeholder:text-zinc-600 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20" /></label><div className="rounded-xl border border-white/15 bg-black px-4 py-4 text-sm text-zinc-400"><span className="inline-flex items-center gap-2"><ShieldCheck size={17} className="text-orange-400" /> Card number, expiry, and CVC will open securely in Stripe.</span></div></div><button type="submit" className="mt-6 flex min-h-14 w-full items-center justify-center gap-2 rounded-xl bg-orange-500 px-5 text-sm font-black tracking-wide text-black transition hover:scale-[1.01] hover:bg-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-200 focus:ring-offset-2 focus:ring-offset-[#111]">PAY NOW WITH STRIPE — SECURE CHECKOUT <ArrowRight size={18} /></button>{submitted && <p role="status" className="mt-4 rounded-xl border border-orange-400/30 bg-orange-500/10 p-4 text-sm leading-relaxed text-orange-100">Checkout is ready to connect. Add your Stripe Payment Link as <code className="rounded bg-black px-1.5 py-0.5">NEXT_PUBLIC_LOYALTY_STRIPE_PAYMENT_LINK</code>, then this button will securely open Stripe without your site ever handling card details.</p>}<p className="mt-4 text-center text-xs text-zinc-500">Payments are processed by Stripe. Five Star Growth never stores card details.</p></form></section>
    </main>
  );
}
