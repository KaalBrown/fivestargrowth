...
import { LoyaltyConfiguration, LoyaltyPricing, loyaltyMonthlyTotal } from "./LoyaltyPricing";
...
const checkoutUrls = {
  starter: process.env.NEXT_PUBLIC_LOYALTY_STARTER_STRIPE_PAYMENT_LINK,
  growth: process.env.NEXT_PUBLIC_LOYALTY_GROWTH_STRIPE_PAYMENT_LINK,
};
...
export function LoyaltyProgramLanding() {
  ...
  const [configuration, setConfiguration] = useState<LoyaltyConfiguration>({ plan: "growth", unlimitedCustomers: false, extraStands: 0 });

  function handleCheckout(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const checkoutUrl = checkoutUrls[configuration.plan];
    if (checkoutUrl) {
      window.location.assign(checkoutUrl);
      return;
    }
    const selectedPlan = configuration.plan === "growth" ? "Growth" : "Starter";
    const extras = [configuration.unlimitedCustomers ? "Unlimited Customers" : "", configuration.extraStands ? `${configuration.extraStands} extra counter stand${configuration.extraStands === 1 ? "" : "s"}` : ""].filter(Boolean).join(", ");
    window.location.assign(`https://wa.me/64204510002?text=${encodeURIComponent(`Hi Five Star Growth, I would like to start the FiveStar Loyalty ${selectedPlan} plan ($${loyaltyMonthlyTotal(configuration)} NZD/month)${extras ? ` with ${extras}` : ""}. Please send me the secure checkout link.`)}`);
    setSubmitted(true);
  }
  ...
  return (
    <main className="overflow-hidden bg-[#080808] text-white selection:bg-orange-500 selection:text-black">
      ...
      <section id="checkout" className="scroll-mt-20 px-5 py-20 sm:px-8 sm:py-28"><div className="mx-auto max-w-5xl text-center"><p className="text-xs font-bold uppercase tracking-[.2em] text-orange-400">Simple pricing. No decision fatigue.</p><h2 className="mt-4 text-3xl font-black tracking-[-.05em] sm:text-5xl">Start simple. Add capacity only when you need it.</h2><p className="mx-auto mt-4 max-w-2xl text-lg leading-relaxed text-zinc-400">Both plans already include your first Smart Counter Stand and room for 500 active loyalty members — exactly what most local shops need.</p><div className="mt-6 flex flex-wrap justify-center gap-3"><span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/35 bg-emerald-400/10 px-4 py-2 text-sm font-bold text-emerald-200"><ShieldCheck size={16} /> 30-day money-back guarantee</span><span className="inline-flex items-center gap-2 rounded-full border border-orange-400/35 bg-orange-400/10 px-4 py-2 text-sm font-bold text-orange-200"><Truck size={16} /> Express 48h shipping</span></div></div><form onSubmit={handleCheckout} className="mx-auto mt-10 max-w-4xl rounded-3xl border border-orange-500/45 bg-[#111] p-5 text-left shadow-[0_0_55px_rgba(255,107,0,.13)] sm:p-8"><div className="flex items-center justify-between border-b border-white/10 pb-5"><div><p className="font-black">Build your FiveStar Loyalty package</p><p className="mt-1 text-sm text-zinc-400">One clear plan. Only the upgrades you genuinely need.</p></div><CreditCard className="text-orange-400" /></div><div className="mt-6"><LoyaltyPricing value={configuration} onChange={setConfiguration} /></div><div className="mt-6 rounded-xl border border-white/15 bg-black px-4 py-4 text-sm text-zinc-400"><span className="inline-flex items-center gap-2"><ShieldCheck size={17} className="text-orange-400" /> Your selected total is ${loyaltyMonthlyTotal(configuration)} NZD / month. We will send the right secure checkout link for your exact setup.</span></div><button type="submit" className="mt-6 flex min-h-14 w-full items-center justify-center gap-2 rounded-xl bg-orange-500 px-5 text-sm font-black tracking-wide text-black transition hover:scale-[1.01] hover:bg-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-200 focus:ring-offset-2 focus:ring-offset-[#111]">REQUEST SECURE CHECKOUT — ${loyaltyMonthlyTotal(configuration)} NZD / MO <ArrowRight size={18} /></button>{submitted && <p role="status" className="mt-4 rounded-xl border border-orange-400/30 bg-orange-500/10 p-4 text-sm leading-relaxed text-orange-100">Your selected package has been prepared. We’ll send the secure checkout link for the exact plan and add-ons you chose.</p>}<p className="mt-4 text-center text-xs text-zinc-500">Payments are processed securely. Five Star Growth never stores card details.</p></form></section>
    </main>
  );
}
