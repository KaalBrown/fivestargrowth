...
export function ServiceVisual({ serviceKey }: { serviceKey: ServiceKey }) {
  ...
  return (
    <div className="relative overflow-hidden border border-white/15 bg-[#101010] p-5 text-white shadow-[10px_10px_0_#101010] sm:p-7">
      <div
        className="absolute -right-12 -top-14 h-44 w-44 rounded-full blur-3xl"
        style={{ backgroundColor: service.colour, opacity: 0.42 }}
      />
      <div className="relative flex min-h-44 flex-col justify-between sm:min-h-48">
        <div className="flex items-start justify-between">
          <span className="fsg-mono text-xs uppercase tracking-[.14em] text-white/75">
            Five Star Growth / service
          </span>
          <Star
            className="h-4 w-4"
        ...
        <div className="flex items-end justify-between gap-5">
          <Icon
            className="h-20 w-20 stroke-[1.25] sm:h-24 sm:w-24"
            style={{ color: service.colour }}
          />
          <div className="max-w-44 border-l border-white/20 pl-4 text-right text-sm leading-5 text-white/90">
            Built around visible proof, clear actions, and sustainable local
            growth.
          </div>
        </div>
      </div>
      ...
      <section className="border-b border-black/15 px-5 py-16 lg:px-9 lg:py-24">
        <div className="mx-auto grid max-w-[1240px] items-center gap-10 lg:grid-cols-[1.05fr_.95fr]">
          <div>
            ...
            <h1 className="mt-5 max-w-3xl text-5xl font-extrabold leading-[.94] tracking-[-.07em] sm:text-6xl">
              {service.title}
            </h1>
            ...
            <Button
              href={`/contact?reason=video-audit&cta=service-${service.key}&service=${service.key}`}
              variant="glow"
              className="mt-8"
            >
          ...
          <ServiceVisual serviceKey={serviceKey} />
        </div>
      </section>
      <section className="px-5 py-16 lg:px-9 lg:py-24">
        <div className="mx-auto max-w-[1240px]">
          <div className="grid gap-4 lg:grid-cols-2">
            <article className="border border-black/15 p-5 sm:p-6">
              <p
                className="fsg-mono text-xs uppercase tracking-[.14em]"
                style={{ color: service.colour }}
              >
              ...
              <p className="mt-5 text-xl font-bold leading-8 tracking-[-.04em]">
                {service.whoItsFor}
              </p>
            </article>
            <article className="border border-black/15 p-5 sm:p-6">
          ...
          <div className="mt-16 grid gap-10 lg:grid-cols-[.8fr_1.2fr]">
            <div>
              ...
              <h2 className="mt-4 text-4xl font-extrabold leading-[.95] tracking-[-.06em]">
                A focused system, not a vague retainer.
              </h2>
            </div>
            <ul className="grid gap-3 sm:grid-cols-2">
              {service.includes.map((item, index) => (
                <li
                  key={item}
                  className="border border-black/15 p-5 text-sm leading-6"
                >
                  <span
          ...
          <div className="mt-20 grid gap-10 lg:grid-cols-[1fr_.8fr]">
            <div>
              ...
              <div className="mt-4 border-y border-black/15">
                {service.faqs.map((faq) => (
                  <details
                    key={faq.question}
                    className="group border-b border-black/15 py-5 last:border-0"
                  >
                    <summary className="cursor-pointer list-none pr-7 text-lg font-extrabold marker:hidden">
                      {faq.question}
                      <span className="float-right transition group-open:rotate-45">
                        +
                      </span>
                    </summary>
            ...
            <aside className="self-end bg-[#101010] p-6 text-white">
              <p className="text-2xl font-extrabold leading-tight tracking-[-.04em]">
                Want a clearer view of your best next move?
              </p>
              <p className="mt-4 text-base leading-6 text-white/80">
                We will review your current online presence and send practical
                priorities for your business.
              </p>
              <Button
                href={`/contact?reason=video-audit&cta=service-faq-${service.key}&service=${service.key}`}
                variant="glow"
...
export function SocialMarks() {
  return (
    <div className="flex items-center gap-3 text-white/70">
      ...
      <span className="text-[10px] font-extrabold tracking-[.12em]">META</span>
    </div>
  );
}
export function ServicesCta({
  href = "/contact?reason=video-audit&cta=services",
}: {
  href?: string;
}) {
  return (
    <Button href={href} variant="glow">
      Request Free Video Audit
    </Button>
  );
}
